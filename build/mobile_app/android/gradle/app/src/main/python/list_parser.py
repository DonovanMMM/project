...
TITLES_OF_CHRIST_FILEPATH = resource_path("titles_of_christ.txt")  # List of titles of Christ
...
def should_show_progress() -> bool:
    # Disable terminal progress rendering on mobile runtimes.
...
def _build_progress(description: str) -> Progress:
    return Progress(
...
def titles_of_christ_parser(filepath):
    ...
    progress_bar = _build_progress("Parsing Titles of Christ...") if show_progress else None
    try:
        names = []
        with open(filepath, "r", encoding="utf-8") as titles:
            all_titles = titles.readlines()
            iterator = progress_bar.track(all_titles, total=len(all_titles)) if progress_bar else all_titles
            if progress_bar:
                progress_bar.start()
...
def book_of_mormon_parser():
    ...
    show_progress = should_show_progress()
    progress_bar = _build_progress("Parsing Book of Mormon...") if show_progress else None

    with open(BOOK_OF_MORMON_FILEPATH, "r", encoding="utf-8") as book:
        all_text = book.readlines()
        iterator = progress_bar.track(all_text, total=len(all_text)) if progress_bar else all_text
        if progress_bar:
            progress_bar.start()
        for text in iterator:
